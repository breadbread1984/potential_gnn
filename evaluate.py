...
def main(unused_argv):
  ...
  for data in tqdm(dataloader):
    ...
    pred_excs.append(torch.squeeze(pred_exc, dim = -1).detach().cpu())
    pred_vxcs.append(torch.squeeze(pred_vxc, dim = -1).detach().cpu())
    ...
    true_excs.append(true_exc.detach().cpu())
    true_vxcs.append(true_vxc.detach().cpu())
    ...
    pred_vxcs_der.append(pred_vxc.detach().cpu())
