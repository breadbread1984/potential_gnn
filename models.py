...
class CustomAggregation(Aggregation):
  ...
  def forward(self, x, source_x, dest, x_pos = None, source_x_pos = None):
    ...
    dest_x = torch.scatter_add(x, dest1, dim = 0, input = torch.zeros_like(dest1, dtype = x.dtype, device = x.device)) # dest_x.shape = (edge_num, channel)
    dest2 = torch.tile(torch.unsqueeze(dest, dim = -1), (1, x_pos.shape[1])) # dest2.shape = (edge_num, 3)
    dest_x_pos = torch.scatter_add(x_pos, dest2, dim = 0, input = torch.zeros_like(dest2, dtype = x_pos.dtype, device = x_pos.device)) # dest_x_pos.shape = (edge_num, 3)
...
class CustomConv(MessagePassing):
  ...
  def aggregate(self, x, edge_index, x_pos):
    ...
    source_x = torch.scatter_add(x, source1, dim = 0, input = torch.zeros_like(source1, dtype = x.dtype, device = x.device)) # source_x.shape = (edge_num, channels)
    source2 = torch.tile(torch.unsqueeze(source, dim = -1), (1, x_pos.shape[1])) # source2.shape = (edge_num, 3)
    source_x_pos = torch.scatter_add(x_pos, source2, dim = 0, input = torch.zeros_like(source2, dtype = x_pos.dtype, device = x_pos.device)) # source_x_pos.shape = (edge_num, 3)
